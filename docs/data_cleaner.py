...
def clean_dataset(df, columns_to_check=None, drop_duplicates=True, fill_missing='mean'):
    ...
    columns_to_check (list, optional): Columns to check for duplicates. If None, uses all columns.
    drop_duplicates (bool): Whether to drop duplicate rows. Default is True.
    fill_missing (str or value): Method to fill missing values. Can be 'mean', 'median', 'mode', or a scalar value.
...
def clean_dataset(df, drop_duplicates=True, fill_missing='mean'):
    ...
    if len(numeric_cols) > 0:
        cleaned_df[numeric_cols] = cleaned_df[numeric_cols].round(4)
