...
def validate_dataframe(df, required_columns=None):
    ...
    if null_counts.any():
        validation_results['warnings'].append(f"Found {null_counts.sum()} missing values across {len(null_counts[null_counts > 0])} columns")
...
def save_cleaned_data(data, filename, format='csv'):
    ...
    if format == 'csv':
        ...
        raise ValueError("Invalid format. Choose 'csv', 'parquet', or 'excel'")
