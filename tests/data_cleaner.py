...
def create_cleaning_report(data, cleaned_data):
    ...
    for col in numeric_cols:
        if col in data.columns and col in cleaned_data.columns:
...
if __name__ == "__main__":
    ...
    print(f"Rows removed: {report['removed_rows']} ({report['removed_percentage']:.2f}%)")
import pandas as pd
...
def clean_dataset(df, drop_duplicates=True, fill_missing=None):
    ...
    fill_missing (str or None): Method to fill missing values.
        Options: 'mean', 'median', 'mode', or None to drop rows with missing values.
