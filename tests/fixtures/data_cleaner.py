    ...
    for column in numeric_cols:
        ...
        if outliers.any():
            ...
            cleaned_df.loc[outliers, column] = np.where(
                ...
                upper_bound,
                np.where(
                    cleaned_df.loc[outliers, column] < lower_bound,
                    lower_bound,
                    cleaned_df.loc[outliers, column]
                )
            )
...
def get_summary_statistics(df):
    ...
    summary.loc['skewness'] = df.skew(numeric_only=True)
    summary.loc['kurtosis'] = df.kurtosis(numeric_only=True)
    return summary
...
if __name__ == "__main__":
    ...
    stats = get_summary_statistics(cleaned)
    print("\nSummary statistics:")
    print(stats)
import numpy as np
import pandas as pd
...
def validate_data(data, required_columns=None, allow_nan_ratio=0.1):
    ...
    is_valid = True
    if 'missing_columns' in validation_report and validation_report['missing_columns']:
        is_valid = False
    
    high_nan_columns = [
        col for col, info in nan_report.items() 
        if info['status'] == 'high_nan'
    ]
    if high_nan_columns:
        is_valid = False
