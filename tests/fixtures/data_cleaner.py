...
def clean_dataset(df: pd.DataFrame, operations: dict) -> pd.DataFrame:
    ...
    if operations.get('remove_duplicates', False):
        subset = operations.get('duplicate_subset', None)
        cleaner.remove_duplicates(subset)
    
    if 'missing_strategy' in operations:
        strategy = operations.get('missing_strategy')
        fill_value = operations.get('fill_value')
        cleaner.handle_missing_values(strategy, fill_value)
    
    if 'normalize_columns' in operations:
        cleaner.normalize_numeric(operations['normalize_columns'])
