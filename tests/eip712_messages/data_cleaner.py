...
def process_data_pipeline(data, config):
    """
    Execute data processing pipeline based on configuration.
    ...
    if config.get('remove_outliers', False):
        columns = config.get('outlier_columns', result.select_dtypes(include=[np.number]).columns.tolist())
        for col in columns:
            if col in result.columns:
                result = remove_outliers_iqr(result, col, config.get('outlier_factor', 1.5))
    
    if config.get('normalize', False):
        columns = config.get('normalize_columns', result.select_dtypes(include=[np.number]).columns.tolist())
        for col in columns:
            if col in result.columns:
                result[col] = normalize_minmax(result, col)
    
    if config.get('standardize', False):
        columns = config.get('standardize_columns', result.select_dtypes(include=[np.number]).columns.tolist())
        for col in columns:
            if col in result.columns:
