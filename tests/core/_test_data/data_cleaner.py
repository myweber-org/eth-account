...
def log_transform_skewed(df, skewed_cols):
    ...
    Parameters:
    df (pd.DataFrame): Input dataframe
    skewed_cols (dict): Dictionary from detect_skewed_columns function
...
def clean_dataset(df, outlier_threshold=1.5, normalize_method='minmax', handle_skewness=True):
    """
    ...
    handle_skewness (bool): Whether to apply log transformation to skewed columns
    ...
    if handle_skewness:
        skewed = detect_skewed_columns(df_clean)
        if skewed:
