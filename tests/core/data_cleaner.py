...
if __name__ == "__main__":
    ...
    print(f"\nDataset validation status: {'PASS' if validation_passed else 'FAIL'}")
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    if cleaned is not None:
        ...
        print(f"Outliers in column 'A': {cleaned[outlier_mask]['A'].tolist()}")import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    if validate_dataframe(df, ['feature_a', 'feature_b']):
        cleaned_df = clean_dataset(df, ['feature_a', 'feature_b'])
        print("Cleaned dataset shape:", cleaned_df.shape)
        print("Feature ranges after cleaning:")
        print(f"feature_a: [{cleaned_df['feature_a'].min():.3f}, {cleaned_df['feature_a'].max():.3f}]")
        print(f"feature_b: [{cleaned_df['feature_b'].min():.3f}, {cleaned_df['feature_b'].max():.3f}]")
import pandas as pd
