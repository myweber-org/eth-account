...
def create_sample_data():
    ...
    data['income'][outliers] = data['income'][outliers] * 3
...
if __name__ == "__main__":
    ...
    print(cleaner.get_clean_data().head())import numpy as np
import pandas as pd
...
def clean_dataset(df, drop_duplicates=True, handle_nulls='drop', null_threshold=0.5):
    ...
    if handle_nulls == 'drop':
        ...
        print("Dropped all rows with null values")
        ...
        raise ValueError("handle_nulls must be 'drop', 'fill', or 'threshold'")
...
def validate_dataframe(df):
    """
    Perform basic validation checks on the cleaned dataframe.
    """
    if df is None:
        return False
