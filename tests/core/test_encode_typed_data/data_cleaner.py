...
def clean_dataset(df, numeric_columns=None, outlier_columns=None, 
    ...
    Args:
        df (pd.DataFrame): Input DataFrame
        numeric_columns (list): Columns to consider numeric (if None, auto-detect)
...
if __name__ == "__main__":
    ...
    print(f"Original: {len(sample_data)} rows, Cleaned: {len(cleaned)} rows")import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    print(calculate_basic_stats(cleaned_data, 'values'))def remove_duplicates(sequence):
    seen = set()
    result = []
    for item in sequence:
        if item not in seen:
