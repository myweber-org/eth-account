...
def clean_dataframe(df, drop_duplicates=True, fill_missing=True):
    ...
    if fill_missing:
        ...
        print("Filled missing values in numeric columns with mean and categorical columns with 'Unknown'.")
...
if __name__ == "__main__":
    ...
    cleaned_df = clean_csv_data('sample_data.csv', missing_strategy='median', columns_to_drop=['D'])
    ...
    for col in numeric_cols:
        if outlier_method == 'iqr':
            ...
            df_clean[col] = np.where(z_scores > 3, 
                                    np.where(df_clean[col] > mean, mean + 3*std, mean - 3*std),
                                    df_clean[col])
