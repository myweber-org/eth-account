...
def validate_dataframe(df, check_duplicates=True, check_missing=True):
    ...
    if check_missing:
        ...
        if total_missing > 0:
            print(f"Warning: Found {total_missing} missing values across {len(validation_results['missing_by_column'])} columns.")
            for col, count in validation_results['missing_by_column'].items():
...
#     print("\nCleaned DataFrame:")
#     print(cleaned_df)
#     print("\nValidation result:", validate_dataframe(cleaned_df, ['name', 'age']))
import pandas as pd
import numpy as np
from scipy import stats
...
if __name__ == "__main__":
    ...
    cleaned_df = clean_dataset(input_path, output_path)import numpy as np
import pandas as pd
...
def main():
    sample_data = {
        'feature_a': [10, 12, 9, 15, 100, 11, 13, 8, 14, 12],
        'feature_b': [5, 6, 4, 7, 50, 5, 6, 4, 7, 5],
    ...
    print("\nCleaned dataset:")
    print(cleaned)
    print("\nCleaning complete. Outliers removed and features normalized.")
...
def clean_dataset(data, numeric_columns, outlier_method='iqr', normalize=False, standardize=False):
    """
...
if __name__ == "__main__":
    ...
    cleaned = clean_dataset(sample_data, ['A', 'B', 'C'], outlier_method='iqr')
