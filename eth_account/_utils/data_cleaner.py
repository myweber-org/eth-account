...
if __name__ == "__main__":
    ...
    print(f"After mean imputation: {filled_df.shape}")import pandas as pd
import sys
...
def clean_dataset(data, numeric_columns=None, outlier_method='iqr', normalize_method='minmax'):
    ...
    Args:
        data: pandas DataFrame
        numeric_columns: list of numeric columns to clean (default: all numeric columns)
...
def validate_data(data, require_columns=None, allow_nan_ratio=0.1):
    ...
    for column in data.columns:
        ...
        if nan_ratio > allow_nan_ratio:
            validation_result['high_nan_columns'].append(column)
            validation_result['is_valid'] = False
        
        if data[column].dtype in [np.float64, np.int64]:
            if data[column].nunique() == 1:
                validation_result['zero_variance_columns'].append(column)
                validation_result['is_valid'] = False
