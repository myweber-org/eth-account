...
def main():
    ...
    stats_before = calculate_summary_statistics(df, 'value')
    ...
    stats_after = calculate_summary_statistics(cleaned_df, 'value')
...
if __name__ == "__main__":
    ...
    # Remove outliers
    df_no_outliers = remove_outliers_iqr(df_clean, columns=['A', 'B'])
    print("\nAfter removing outliers:")
    print(df_no_outliers)
    
    # Normalize data
    df_normalized = normalize_data(df_no_outliers, method='minmax')
    print("\nAfter normalization:")
    print(df_normalized)
    
    # Validate data
    is_valid, message = validate_data(df_normalized, required_columns=['A', 'B', 'C'])
    print(f"\nData validation: {is_valid} - {message}")
    
    # Get summary
    summary = get_data_summary(df_normalized)
    print("\nData summary:")
    ...
    for col in skewed_columns:
        if col in dataframe.columns:
            ...
            if min_val <= 0:
                ...
                transformed_df[col] = np.log(dataframe[col] + offset)
            else:
                transformed_df[col] = np.log(dataframe[col])
...
def clean_numeric_column(df, column_name, fill_method='mean'):
    ...
    Args:
        ...
        fill_method: method to fill missing values ('mean', 'median', 'mode', 'zero')
    ...
    if fill_method == 'mean':
        ...
        fill_value = df_clean[column_name].mode()[0]
    ...
    if normalize_text:
        ...
        for col in text_columns:
            cleaned_df[col] = cleaned_df[col].apply(
                lambda x: _normalize_string(x) if isinstance(x, str) else x
            )
