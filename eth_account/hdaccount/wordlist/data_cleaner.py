...
def clean_dataset(df, drop_duplicates=True, fill_missing='mean'):
    ...
    if missing_count > 0:
        ...
        if fill_missing == 'drop':
            ...
            print("Filled missing values with column modes (objects) or means (numeric)")
    
    # Reset index after cleaning
    cleaned_df = cleaned_df.reset_index(drop=True)
    
    final_rows = len(cleaned_df)
    print(f"Final dataset has {final_rows} rows and {len(cleaned_df.columns)} columns")
...
def sample_data_cleaning():
    ...
    data = {
        ...
        'age': [25, 30, 35, 35, 28, None, 42, 33, 29, 31],
        'score': [85.5, 92.0, 78.5, 78.5, 88.0, 95.5, 72.0, 81.5, None, 89.0]
    }
...
if __name__ == "__main__":
    cleaned_data = sample_data_cleaning()import numpy as np
import pandas as pd
...
def process_dataset(file_path, column_to_clean):
    """
    Main function to load data, clean outliers, and return processed data.
...
if __name__ == "__main__":
    ...
    cleaned_data, original_stats, cleaned_stats = process_dataset('sample_data.csv', 'values')
    
    print("Original statistics:", original_stats)
    print("Cleaned statistics:", cleaned_stats)
    print(f"Removed {len(sample_data) - len(cleaned_data)} outliers")import pandas as pd
import numpy as np
...
def clean_dataframe(df, text_column):
    ...
    df_clean = df_clean[df_clean[text_column].notna() & (df_clean[text_column] != '')]
